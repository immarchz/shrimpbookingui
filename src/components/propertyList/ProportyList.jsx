    ...
    <div className="propertyList">
      {propertylist.map((item) => (
        <div className="propertyContainer">
          <img src={item.picture} alt="" className="propertyImage" />
          <h2 className="propertyName">{item.name} </h2>
          <h3 className="propertyCount">{item.count}</h3>
        </div>
      ))}
    </div>
  );
